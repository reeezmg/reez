...
function DriverInfo() {
  ...
  useEffect(() => {
    const fetchDriverInfo = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/drivers/getdriverinfo`, {
  ...
  const handleRemovePickupPoint = async (id, index) => {
    if (id) {
      ...
      try {
        await axios.delete(`http://localhost:5000/api/drivers/pickuppoint/${id}`, {
          withCredentials: true,
        });
  ...
  const handleSaveInfo = async () => {
    try {
      ...
      await axios.put('http://localhost:5000/api/drivers/update', driverInfo, {
        withCredentials: true,
      });
